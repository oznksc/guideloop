...
/** Strip empty optional fields for clean export. */
...
/**
 * Parse JSON array (or `{ steps: [...] }`) into builder steps.
 */
export function importFromJson(raw: string): ImportResult {
  ...
  if (!list) {
    return {
      ok: false,
      error: 'Expected a JSON array of steps or { "steps": [...] }',
    };
  }
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    const additionalTargets = Array.isArray(rec.additionalTargets)
